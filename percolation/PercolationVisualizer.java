...
public class PercolationVisualizer {
    ...
    public static void draw(Percolation percolation, int n) {
        ...
        // draw n-by-n grid
        // I believe that this drawing of the grid assumes a 0 -> n - 1 indexing, when in reality the autograder and specification has a 1 -> n indexing
        for (int row = 0; row < n; row++) {
            for (int col = 0; col < n; col++) {
                if (percolation.fullzero(row,
                                         col)) { // when using the visualizer, I got rid of the private access modifiers so that the visualizer could see the methods
                    StdDraw.setPenColor(StdDraw.BOOK_LIGHT_BLUE);
                }
                else if (percolation.isopenzero(row,
                                                col)) { // when using the visualizer, I got rid of the private access modifiers so that the visualizer could see the methods
