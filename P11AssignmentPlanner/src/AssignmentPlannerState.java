...
//
// Title: P11 Assignment Planner State
  ...
  MAIN_MENU {
    ...
    public AssignmentPlannerState runState(Scanner scanner, AssignmentQueue queue) {
      ...
      if (num == 1) {
        ...
        if (queue.isEmpty()) {
          System.out.println("Warning, cannot peek an empty AssignmentQueue");
        } else {
        ...
        // Make sure to consider the case when queue is empty
        if (queue.isEmpty()) {
          System.out.println("Warning, cannot dequeue from an empty AssignmentQueue");
        } else {
